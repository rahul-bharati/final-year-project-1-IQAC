<?php
    ...
    if((move_uploaded_file($_FILES['image1']['tmp_name'], $target1)) && (move_uploaded_file($_FILES['image2']['tmp_name'], $target2)) && (move_uploaded_file($_FILES['image3']['tmp_name'], $target3)) && (move_uploaded_file($_FILES['image4']['tmp_name'], $target4)) && (move_uploaded_file($_FILES['report']['tmp_name'], $target_report)))
    {
        ...
        $sql = "insert into iqac.guest_lectures values('','$title','$subtitle','$description','$date','$name','$topic','$contact','$target_report','$target1','$target2','$target3','$target4');";
        // echo $con->error."</br>";
        // echo $sql;
        if($con->query($sql))
        {
            $lastid = $con->insert_id;
            $sql =  "insert into iqac.report values('','$lastid','". $_SESSION["dept"] ."',3,now(),'unapproved')";
            if($con->query($sql))
            {
                $msg = "File Uploaded Successfully. <a href="."../view/view.php?id=$lastid&type=3".">Click Here</a> print it.";
            }
        }
    }
    echo "$msg";
