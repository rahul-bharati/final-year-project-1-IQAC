<?php
    ...
    if((move_uploaded_file($_FILES['image1']['tmp_name'], $target1)) && (move_uploaded_file($_FILES['image2']['tmp_name'], $target2)) && (move_uploaded_file($_FILES['image3']['tmp_name'], $target3)) && (move_uploaded_file($_FILES['image4']['tmp_name'], $target4)) && (move_uploaded_file($_FILES['report']['tmp_name'], $target_report)))
    {
        ...
        $sql = "insert into iqac.events values('','$title','$subtitle','$description','$date','$target_report' ,'$target1','$target2','$target3','$target4');";
        if($con->query($sql))
        {
            $lastid = $con->insert_id;
            $sql =  "insert into iqac.report values('','$lastid','".$_SESSION["dept"]."',5,now(),'unapproved')";
            if($con->query($sql))
            {
                $msg = "File Uploaded Successfully. <a href="."../view/view.php?id=$lastid&type=5".">Click Here</a> print it.";
            }
        }
    }
    echo "$msg";
